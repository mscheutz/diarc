...
import edu.tufts.hrilab.diarcros.pr2.LGripperControllerGripperActionNode;
import org.ros.actionlib.state.SimpleClientGoalState;
import org.ros.exception.RosException;

import java.util.concurrent.TimeUnit;

public class PR2LeftGripper extends GenericManipulator {
  LGripperControllerGripperActionNode l_gripper;

  public PR2LeftGripper() {
    super();
    l_gripper = LGripperControllerGripperActionNode.getInstance();

    l_gripper.waitForNode();
  ...
  public boolean moveGripper(float position) {
    try {
      ...
      l_gripper.sendGoal(command_goal);
      log.debug("Left gripper moving...");
      l_gripper.waitForResult(3000l, TimeUnit.MILLISECONDS);
      log.debug("Left gripper done.");
      result = l_gripper.getResult();
      if (result == null) {
        l_gripper.cancelAllGoals();
      ...
      state = l_gripper.getState();
      log.debug(String.format("l_gripper goal state: %s.", state.getState().toString()));
  ...
  public float getCurrentGripperPosition() {
    try {
      return (float) l_gripper.getResult().getPosition();
  ...
  public float getCurrentGripperEffort() {
    try {
      return (float) l_gripper.getResult().getEffort();
    } catch (RosException e) {
